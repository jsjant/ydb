 ...
 *
 * @note Only a whole path component is ever stripped: the path /Root/db10/table is
 *       NOT considered to be within the database /Root/db1.
 *
 * @note A path, which is not within the database, is returned as is. That should
 *       never happen, but an odd looking "table" label is much easier to notice
 *       than silently dropped counters.
 */
