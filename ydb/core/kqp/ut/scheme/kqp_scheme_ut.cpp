...
Y_UNIT_TEST_SUITE(KqpScheme) {
    ...
    Y_UNIT_TEST(TableMetricsLevelInvalidValue) {
        ...
        for (const auto* level : {"\"DISABLED\"", "0", "\"NOPE\"", "9", "\"UNSPECIFIED\""}) {
    ...
    Y_UNIT_TEST_TWIN(AlterDatabaseTablesMetricsLevel, EnableDataShardDetailedMetrics) {
        ...
        // Test: clear with "UNSPECIFIED"
        {
            auto result = session.ExecuteQuery(R"(
                    ALTER DATABASE `/Root/Test` SET (TABLES_METRICS_LEVEL = "UNSPECIFIED");
                )", NQuery::TTxControl::NoTx()
            ).ExtractValueSync();
            UNIT_ASSERT_C(result.IsSuccess(), result.GetIssues().ToString());
        }
        {
            const auto level = GetTablesMetricsLevel(env.GetServer(), "/Root/Test");
            UNIT_ASSERT_VALUES_EQUAL(level, ui32(NKikimrSchemeOp::TTableDetailedMetricsSettings::MetricsLevelUnspecified));
        }

        // DISABLED and numeric 0 are unsupported and must keep the current default.
        {
            auto result = session.ExecuteQuery(R"(
                    ALTER DATABASE `/Root/Test` SET (TABLES_METRICS_LEVEL = "PARTITION");
                )", NQuery::TTxControl::NoTx()
